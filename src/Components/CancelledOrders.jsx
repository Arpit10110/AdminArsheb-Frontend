...
const CancelledOrders = () => {
    ...
    const columns = [
      ...
      const pendingdata = async()=>{
        try {
            const {data} = await axios.get(`${import.meta.env.VITE_Port}/cancel`);
    ...
    <div className='w-full pt-[2rem] flex flex-wrap justify-around gap-y-[3rem] pb-[5rem] mt-[3rem]' >
               ...
               <TableContainer sx={{ maxHeight: 'none' }}>
                ...
                PendingList.map((i,index)=>{
                    return(
                        <Datatable key={index} date={i.orderdate} id={i._id} customername={i.user.name} sn={index+1} total={i.total} payment={i.payment} address={i.address} status={"🔴Cancelled"}/>
                    )
                })
            }
                    </Table>
